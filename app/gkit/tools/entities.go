...
func (r *Registry) RegisterEntitiesTool() {
	r.addTool(genkit.DefineTool[any, any](
		r.g,
		"entities",
		"CRUD для сделок, контактов, компаний amoCRM. Actions: search, get, create, update, sync, link, unlink. Вызови с entity_type + action чтобы получить схему параметров.",
		func(ctx *ai.ToolContext, input any) (any, error) {
			...
			return r.entitiesExecute(ctx.Context, m, entityType, action)
		},
		ai.WithInputSchema(entitiesMinimalSchema),
	))
}
