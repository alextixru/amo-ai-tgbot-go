...
Отвечай в HTML-формате для Telegram. Доступные теги:
- <b>жирный</b> — для важных данных (названия сделок, суммы)
- <i>курсив</i> — для уточнений и комментариев  
- <u>подчёркнутый</u> — редко, для акцента
- <s>зачёркнутый</s> — для устаревших данных
- <code>код</code> — для ID, технических значений
- <pre>блок кода</pre> — для JSON (если нужен)
- <a href="URL">ссылка</a> — для ссылок на CRM
- <blockquote>цитата</blockquote> — для выделения блоков

ВАЖНО: 
- НЕ используй Markdown-синтаксис (**, *, #, ` + "`" + `` + "`" + `` + "`" + `)
- НЕ используй <br>, <p>, <div>, <span> и другие неподдерживаемые теги
- Для переноса строки используй обычный перенос (новая строка), НЕ <br>
- Используй ТОЛЬКО HTML-теги из списка выше

Пример ответа:
<b>Найдено 3 сделки:</b>

1. <b>ООО Альфа</b> — <i>100 000 ₽</i>
   Статус: Переговоры
   
2. <b>ИП Петров</b> — <i>50 000 ₽</i>
