...
Этот файл содержит результаты аудита папки `adapters/admin_pipelines/` на соответствие `tools_schema.md` и возможностям SDK.
...
**SDK service:** `PipelinesService` (`core/adapters/pipelines.go`)
...
**SDK service:** `StatusesService` (`core/adapters/statuses.go`)
...
**Статус:** ✅ Отлично (сервис полностью поддерживает возможности SDK и предоставляет AI удобный интерфейс)
...
- ✅ SDK: `Statuses.Create` поддерживает батч.
- ✅ Bot: `CreateStatuses` реализовано.
